...
7.	[GPSD](http://aosabook.org/en/gpsd.html) by	[Eric Raymond](http://www.catb.org/esr/writings/taoup/html/) ... is a suite of tools for managing collections of GPS devices and other sensors related to navigation and precision timekeeping, including marine AIS (Automatic Identification System) radios and digital compasses. GPSD is widely deployed on laptops, smartphones, and autonomous vehicles including self-driving automobiles and robot submarines.  GPSD has historically had an exceptionally low defect rate in it code, as measured both by auditing tools such as splint, valgrind, and Coverity and by the incidence of bug reports on its tracker and elsewhere. The culture of the GPSD project is about being very aggressive about incorporating technology for automated testing.  So, the clean code of GPSD is not an accident but a matter of test driven design and fastidiously following appropriate best practices and patterns. As another example, the GPSD architecture has been driven by a very conscious and rigorous application of the design pattern of Unix device handling.  Of course, n1ot all patterns apply everywhere, but Unix device handling is the kind of pattern that is beneficial not just in OS kernels but also in userspace programs that are similarly required to deal with varied hardware and protocols.  No project is ever perfect -- in retrospect, we can see that maybe GPSD could have been better designed for extensibility or maybe it would have been better to use client-side libraries rather than exposing the application protocol details, but in general, **sound architecture, good coding practice and a really determined focus on testing did reduce defect rates asymptotically close to zero and the success of the GPSD project illustrates why the most important prerequisite is the discipline to pursue those three things.**
...
14.	[nginx](http://aosabook.org/en/nginx.html) by [Andrey Alexeev](https://www.nginx.com/blog/network-vs-devops-how-to-manage-your-control-issues/#more-2515) ... nginx is a [company](https://www.nginx.com) AND a free, extensible open source web HTTP server [with a fully-supported implementation of HTTP/2](https://www.nginx.com/blog/nginx-plus-r7-released/)], reverse proxy server, a mail proxy server, a generic TCP proxy server and just about any kind of web server [you want to make it into] written by [Igor Sysoev](http://sysoev.ru/en/), a Russian software engineer. Since its public launch in 2004, nginx has focused on high performance, high concurrency and low memory usage. Additional features on top of the web server functionality, like load balancing, caching, access and bandwidth control, and the ability to integrate efficiently with a variety of applications, have helped to make nginx a better choice for the most demanding website architectures. According to [Netcraft](http://news.netcraft.com/), [nginx served or proxied 23.36% busiest sites in September 2015](http://news.netcraft.com/archives/2015/09/16/september-2015-web-server-survey.html). Both Microsoft and Apache lost market share, while [nginx, the company](https://www.nginx.com) was the only major server vendor to increase its market share in both sites and computers this month ... in 2002, when Igor Sysoev started to write nginx, most of the software enabling the Internet already existed and there is still [no shortage of competing alternatives in the web server space](https://en.wikipedia.org/wiki/Comparison_of_web_server_software) ... but there is always room for better idea, particularly in extensible web server foundation technology that better accommodates the needs of third-party application development communities.
...
15.	[Riak and Erlang/OTP](http://aosabook.org/en/riak.html)	by Francesco Cesarini, Andy Gross, and Justin Sheehy ... is a distributed, fault tolerant, open source database that illustrates how to build large scale systems using Erlang/OTP. Smaller and simpler codebases are not only easier to maintain, they often have fewer bugs. Riak is smaller and simpler than other databases becuase it built its own protocols taking advantages of Erlang native support for massively scalable distributed systems and a fundamentally sound asynchronous messaging layer in OTP. Riak offers features that are uncommon in databases, such as high-availability and linear scalability of both capacity and throughput. 
